...
class Quotex(object):
    ...
    async def get_live_assets_with_percentage(self):
        """Fetch live asset names with their percentage changes."""
        live_assets = []
        try:
            instruments = await self.get_instruments()
            for instrument in instruments:
                asset_name = instrument[2].replace("\n", "")
                current_price_data = await self.get_realtime_price(asset_name)

                if current_price_data:
                    # Assuming current_price_data contains 'current_price' and 'previous_price'
                    current_price = current_price_data.get('current_price')
                    previous_price = current_price_data.get('previous_price')
                    ...
                    if previous_price and current_price:
                        ...
                        live_assets.append({
                            'name': asset_name,
                            'percentage_change': round(percentage_change, 2)  # rounding to 2 decimal places
                        })

        except Exception as e:
            logger.error(f"Error fetching live assets: {e}")
    ...
    def get_payment(self):
        ...
        # Ensure self.api.instruments is not None
        if self.api.instruments is None:
            print("Error: Instruments data is not available.")
            return {}  # Return an empty dictionary or handle this case as needed

        assets_data = {}
        for i in self.api.instruments:
            # Assuming i has at least 19 elements
    ...
    async def start_remaing_time(self):
        ...
        while remaing_time >= 0:
            remaing_time -= 1
            print(f"\rStaying {remaing_time if remaing_time > 0 else 0} seconds ...", end="")
    ...
    def macd(self, data, short_window=12, long_window=26, signal_window=9):
        ...
        return macd_line.tolist(), signal_line.tolist(), macd_hist.tolist()
